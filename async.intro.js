...

{
        ...
        try{
                ...
                async function CallValidateApi(){
                        let TokenOfValidation = await ValidateApi(username,passward); 
                        let Content = await LoginApi(username,passward,TokenOfValidation);
                        DisplayApi(Content);
                }
        }
        catch(err){
        ...
        async function ValidateApi(username,passward){
                return new Promise((resolve,reject)=>{
                        setTimeout(()=>{
                                if(username && passward) resolve("valid token");
                                else reject(null);
        ...
        async function LoginApi(username,passward,TokenOfValidation){
                return new Promise((resolve,reject)=>{
                        setTimeout(()=>{
                                if(username && passward && TokenOfValidation)
                                        resolve("content");
